...
void welcome_sendDone(OpenQueueEntry_t* msg, owerror_t error) {
   ...
   if (welcome_vars.desc.confirmable.msg==NULL) {
      ...
      openserial_printInfo(COMPONENT_WELCOME, ERR_DEBUG, 1, 0);
   }
}
...
void welcome_conRetry() {
   ...
   if (welcome_vars.desc.confirmable.msg!=NULL) {
      outcome = opencoap_resend(&welcome_vars.desc.confirmable);
      if (outcome==E_FAIL) {
         openserial_printInfo(COMPONENT_WELCOME, ERR_DEBUG, 1, 1);
      }
   }
}
